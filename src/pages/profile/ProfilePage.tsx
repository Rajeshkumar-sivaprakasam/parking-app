  ...
  const settingsSections = [
    {
      ...
      items: [
        ...
        { icon: Bell, label: 'Notifications', value: notifications ? 'Enabled' : 'Disabled', toggle: true },
      ]
    },
    {
      title: 'Preferences',
      items: [
        { icon: Moon, label: 'Dark Mode', value: darkMode ? 'On' : 'Off', toggle: true },
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="relative z-10 flex items-center gap-6">
          ...
          <div className="flex-1">
            <h3 className="text-2xl font-bold mb-1">Demo User</h3>
            <p className="text-blue-100 mb-3">{t('profile.premiumMember')}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: t('profile.totalBookings'), value: '127', color: 'bg-blue-500' },
          { label: 'Total Spent', value: 'RM 1,245', color: 'bg-purple-500' },
          { label: t('profile.memberSince'), value: 'Jan 2024', color: 'bg-green-500' },
        ].map((stat, index) => (
          <motion.div
            key={stat.label}
            initial={{ opacity: 0, y: 20 }}
              ...
              >
                ...
                {item.toggle ? (
                  <div className={`w-12 h-6 rounded-full transition-colors ${
                    (item.label === 'Notifications' && notifications) || (item.label === 'Dark Mode' && darkMode)
                      ? 'bg-brand-primary' 
      ...
      {/* Logout Button */}
      <button className="w-full bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 py-4 rounded-2xl font-semibold hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors">
        {t('common.logout')}
      </button>
    </div>
  );
